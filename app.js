...
const DEMO_MARKET = [
  ...
  },
  {
    ...
    shares: [
      { title: '진격의 거인 — 에렌 「싸워라」', text: '「싸워라! 싸워라! 싸워라! 그걸 위해서라면 목숨쯤, 아깝지 않다!」— 에렌 예거. 거인의 힘을 각성하며 절규하는 장면. 진격의 거인 최고 명장면으로 꼽히는 시즌3 파트2 클라이맥스.', date: offset(-7) },
      { title: '진격의 거인 — 에렌 「자유」', text: '「이 세계에서 태어난 것만으로, 우리는 자유다.」— 에렌 예거. 벽 밖 세상을 향한 갈망. 자유란 태어날 때부터 주어진 것이라는 에렌의 신념. 시리즈 전체를 관통하는 핵심 대사.', date: offset(-6) },
      { title: '진격의 거인 — 리바이 「선택」', text: '「어떤 선택을 해도 후회는 하게 돼. 어떤 선택이 올바른지는 나도 몰라. 그래도 그게 내 선택이야. 스스로 선택한 거야.」— 리바이 아커만. 전쟁 속 결단의 무게를 담은 명대사.', date: offset(-5) },
      { title: '진격의 거인 — 아르민 「포기」', text: '「뭔가를 포기하지 않으면 아무것도 바꿀 수 없어.」— 아르민 알레르트. 희생 없는 변화는 없다는 진격의 거인의 핵심 메시지. 아르민 특유의 전략적 사고방식을 드러내는 대사.', date: offset(-4) },
      { title: '나루토 — 우치하 이타치', text: '「넌 약하다. 왜 약하냐면, 부족하기 때문이야. 증오가.」— 우치하 이타치가 사스케에게. 어린 사스케에게 증오를 심어준 잔인한 한마디. 훗날 이 대사의 진짜 의미가 밝혀지며 더 큰 감동을 줌.', date: offset(-3) },
      { title: '귀멸의 칼날 — 렌고쿠 쿄주로', text: '「열심히 살아온 자의 불꽃은 영원히 꺼지지 않는다!」— 불주 렌고쿠 쿄주로. 무한열차 편 최후의 명대사. 짧은 등장에도 수많은 팬의 마음에 불꽃을 남긴 캐릭터.', date: offset(-1) },
      { title: '원피스 — 몽키 D. 루피', text: '「난 해적왕이 될 거야!」— 몽키 D. 루피. 세계 최강의 해적을 꿈꾸는 소년의 선언. 원피스를 대표하는 시그니처 대사. 꿈을 당당하게 외치는 것 자체가 루피다움.', date: offset(0) },
    ]
  },
  {
  ...
  },
  {
    ...
    shares: [
      { title: '태양의 후예 — 유시진(송중기)', text: '「난 지금 이 순간에도 선택을 해야 해. 사랑할지, 말지.」 — 유시진. 군인으로서의 임무와 사랑 사이에서 흔들리는 고백. 그 어려운 걸 해냅니다 제가 — 시청률 30% 돌파 명장면.', date: offset(-7) },
      { title: '나의 아저씨 — 이지안(아이유)', text: '「나 좀 싫어해 줄래요? 엄청나게, 끝 간 데 없이, 아주아주 열심히. 나도 아저씨 싫어해 줄게요. 아주아주 열심히.」 — 이지안. 싫어함으로 연결되고 싶은, 역설적인 사랑의 고백.', date: offset(-6) },
      { title: '나의 아저씨 — 박동훈(이선균)', text: '「마음에 걸리는 게 없으면 뭘 죽여도 문제없어. 근데 마음에 걸리면 벌레만 죽여도 탈 나.」 — 박동훈. 양심과 죄책감에 대한 촌철살인. 나의 아저씨 최고 명대사로 꼽히는 줄.', date: offset(-5) },
      { title: '나의 해방일지 — 염미정(김지원)', text: '「저를 숭배해 주세요. 사랑 말고, 숭배.」 — 염미정이 구씨에게. 지치고 무너진 사람이 원하는 건 사랑이 아니라 무조건적인 존중이라는 것. 많은 직장인의 공감을 산 대사.', date: offset(-4) },
      { title: '모범택시 — 김도기(이제훈)', text: '「기억해야 되찾을 수 있는 게 있어. 기억했기 때문에 네놈들 앞에 내가 올 수 있었지.」 — 김도기. 잊지 않고 끝까지 찾아온다는 복수의 선언. 모범택시 시리즈 주제의식이 담긴 명장면.', date: offset(-3) },
      { title: '모범택시 — 택시비 명대사', text: '「내 점괘에 의하면, 당신은 택시비를 아주 비싸게 치르게 될 거야. 지금 받을게, 그 택시비. 네 목숨 값으로!」 — 김도기(시즌2). 악인을 응징하기 직전 날리는 시그니처 대사.', date: offset(-1) },
    ]
  },
  {
...
const SUGGESTED_SECTORS = [
  ...
  '주식투자', '패션/스타일', '부동산', '요리/레시피', '여행꿀팁',
  '영어회화', '심리/자기계발', '드라마명대사', '게임공략', 'IT/개발', '맞춤법/문법',
];
